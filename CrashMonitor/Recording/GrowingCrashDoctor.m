...
@interface  GrowingCrashDoctorParam: NSObject
...
@implementation GrowingCrashDoctorParam
...
@interface GrowingCrashDoctorFunctionCall: NSObject
...
@implementation GrowingCrashDoctorFunctionCall
...
- (NSString*) descriptionForObjCCall
{
    ...
    GrowingCrashDoctorParam* receiverParam = [self.params objectAtIndex:0];
    ...
    GrowingCrashDoctorParam* selectorParam = [self.params objectAtIndex:1];
    if(![selectorParam.type isEqualToString:@GrowingCrashMemType_String])
    {
        return nil;
    }
    ...
    for(int paramNum = 0; paramNum < paramCount; paramNum++)
    {
        ...
        if(paramNum < 2)
        {
            GrowingCrashDoctorParam* param = [self.params objectAtIndex:(NSUInteger)paramNum + 2];
            if(param.value != nil)
            {
                if([param.type isEqualToString:@GrowingCrashMemType_String])
                {
...
- (NSString*) descriptionWithParamCount:(int) paramCount
{
    ...
    {
        GrowingCrashDoctorParam* param = [self.params objectAtIndex:(NSUInteger)i];
...
@implementation GrowingCrashDoctor
...
{
    return [[self crashReport:report] objectForKey:@GrowingCrashField_Error];
}
...
- (NSDictionary*) crashedThreadReport:(NSDictionary*) report
{
    ...
    NSDictionary* crashedThread = [crashReport objectForKey:@GrowingCrashField_CrashedThread];
    if(crashedThread != nil)
    {
        return crashedThread;
    }

    for(NSDictionary* thread in [crashReport objectForKey:@GrowingCrashField_Threads])
    {
        if([[thread objectForKey:@GrowingCrashField_Crashed] boolValue])
        {
            return thread;
        }
    }
...
{
    NSDictionary* backtrace = [threadReport objectForKey:@GrowingCrashField_Backtrace];
    return [backtrace objectForKey:@GrowingCrashField_Contents];
}

- (NSDictionary*) basicRegistersFromThreadReport:(NSDictionary*) threadReport
{
    NSDictionary* registers = [threadReport objectForKey:@GrowingCrashField_Registers];
    NSDictionary* basic = [registers objectForKey:@GrowingCrashField_Basic];
    return basic;
}
...
- (NSDictionary*) lastInAppStackEntry:(NSDictionary*) report
{
    ...
    {
        NSString* objectName = [entry objectForKey:@GrowingCrashField_ObjectName];
...
{
    NSDictionary* machError = [errorReport objectForKey:@GrowingCrashField_Mach];
    if(machError != nil)
    {
        NSString* exceptionName = [machError objectForKey:@GrowingCrashField_ExceptionName];
        return [exceptionName isEqualToString:@"EXC_BAD_ACCESS"];
    }
    NSDictionary* signal = [errorReport objectForKey:@GrowingCrashField_Signal];
    NSString* sigName = [signal objectForKey:@GrowingCrashField_Name];
...
{
    NSDictionary* crashedThread = [self crashedThreadReport:report];
    NSArray* notableAddresses = [crashedThread objectForKey:@GrowingCrashField_NotableAddresses];
    for(NSDictionary* address in [notableAddresses objectEnumerator])
    {
        NSString* type = [address objectForKey:@GrowingCrashField_Type];
        if([type isEqualToString:@"string"])
        {
            NSString* value = [address objectForKey:@GrowingCrashField_Value];
    ...
    {
        NSString* objectName = [entry objectForKey:@GrowingCrashField_ObjectName];
        NSString* symbolName = [entry objectForKey:@GrowingCrashField_SymbolName];
...
- (GrowingCrashDoctorFunctionCall*) lastFunctionCall:(NSDictionary*) report
{
    GrowingCrashDoctorFunctionCall* function = [[GrowingCrashDoctorFunctionCall alloc] init];
    NSDictionary* lastStackEntry = [self lastStackEntry:report];
    function.name = [lastStackEntry objectForKey:@GrowingCrashField_SymbolName];
    ...
    {
        GrowingCrashDoctorParam* param = [[GrowingCrashDoctorParam alloc] init];
        ...
        {
            param.type = [notableAddress objectForKey:@GrowingCrashField_Type];
            NSString* className = [notableAddress objectForKey:@GrowingCrashField_Class];
            NSString* previousClass = [notableAddress objectForKey:@GrowingCrashField_LastDeallocObject];
            NSString* value = [notableAddress objectForKey:@GrowingCrashField_Value];

            if([param.type isEqualToString:@GrowingCrashMemType_String])
            {
                param.value = value;
            }
            else if([param.type isEqualToString:@GrowingCrashMemType_Object])
            {
                param.className = className;
                param.isInstance = YES;
            }
            else if([param.type isEqualToString:@GrowingCrashMemType_Class])
            {
                param.className = className;
                param.isInstance = NO;
            }
...
- (NSString*) zombieCall:(GrowingCrashDoctorFunctionCall*) functionCall
{
...
{
    NSDictionary* stack = [crashedThreadReport objectForKey:@GrowingCrashField_Stack];
    return [[stack objectForKey:@GrowingCrashField_Overflow] boolValue];
}

- (BOOL) isDeadlock:(NSDictionary*) report
{
    NSDictionary* errorReport = [self errorReport:report];
    NSString* crashType = [errorReport objectForKey:@GrowingCrashField_Type];
    return [@GrowingCrashExcType_Deadlock isEqualToString:crashType];
}
...
- (NSString*) diagnoseCrash:(NSDictionary*) report
{
    @try
    {
        NSString* lastFunctionName = [[self lastInAppStackEntry:report] objectForKey:@GrowingCrashField_SymbolName];
        ...
        NSString* crashType = [errorReport objectForKey:@GrowingCrashField_Type];
        if([crashType isEqualToString:@GrowingCrashExcType_NSException])
        {
            NSDictionary* exception = [errorReport objectForKey:@GrowingCrashField_NSException];
            NSString* name = [exception objectForKey:@GrowingCrashField_Name];
            NSString* reason = [exception objectForKey:@GrowingCrashField_Reason]? [exception objectForKey:@GrowingCrashField_Reason]:[errorReport objectForKey:@GrowingCrashField_Reason];
        ...
        GrowingCrashDoctorFunctionCall* functionCall = [self lastFunctionCall:report];
        ...
        {
            uintptr_t address = (uintptr_t)[[errorReport objectForKey:@GrowingCrashField_Address] unsignedLongLongValue];
            if(address == 0)
            {
    ...
    @catch (NSException* e)
    {
        ...
        if(symbols)
        {
            return [NSString stringWithFormat:@"No diagnosis due to exception %@:\n%@\nPlease file a bug report to the GrowingCrash project.", e, symbols];
        }
        return [NSString stringWithFormat:@"No diagnosis due to exception %@\nPlease file a bug report to the GrowingCrash project.", e];
    }
}
